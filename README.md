...
中文：透射式 SXRD/SAXS **样品厚度快速设计器**——从能量、成分、密度与目标透过率，估算该做多厚，并可预算窗片/空气束路。
...
2. **② Sample** — load an **alloy preset** or type composition; prefer **measured density**  
3. **③ Target** — \(T\) (e.g. 0.5) or \(\mu t\) (e.g. 0.7); optional total-stack target  
4. **⑤ Layers** — load a **stack preset** (Kapton/Be/quartz), edit real thicknesses  
5. **Compute thickness** — results + \(T\)–\(t\) curve  
6. **File → Save session** — reproduce the full setup later  
...
- Relative exposure-time estimator  
