...
# -*- coding: utf-8 -*-
"""
MuSlice — SXRD/SAXS Transmission Thickness Designer (GUI)

Design the sample thickness that lets the beam through.

Core physics (Beer–Lambert):
    T = I/I0 = exp(-μ t_eff)
    μ = (μ/ρ)_mix · ρ
    (μ/ρ)_mix = Σ w_i (μ/ρ)_i
    t_eff = t / cos(tilt)
...
from muslice import __app_name__, __tagline__, __version__
from muslice.config_manager import ConfigManager
from muslice.help_panel import HelpPanel
from muslice.presets import alloy_by_id, alloy_choices, stack_by_id, stack_choices
from muslice.session_io import (
    composition_from_pairs,
    load_session,
    pairs_from_composition,
    save_session,
    utc_now_iso,
)
...
APP_TITLE = f"{__app_name__} — SXRD Transmission Thickness Designer"
APP_SUBTITLE = f"{__tagline__}  ·  v{__version__}"
...
def absorption_edge_notes(
    symbols: List[str],
    energy_keV: float,
    window_eV: float = 50.0,
) -> List[str]:
    """
    Soft warning if working energy is near a tabulated absorption edge.
    Feature-detects xraydb APIs; returns [] if unavailable.
    """
    if xraydb is None:
        return []
    notes: List[str] = []
    e_eV = float(energy_keV) * 1000.0
    edge_names = ("K", "L3", "L2", "L1", "M5", "M4", "M3")
    for sym in symbols:
        try:
            # xraydb.xray_edge(element, edge) -> (energy_eV, fyield, jump) in many versions
            edge_fn = getattr(xraydb, "xray_edge", None)
            if edge_fn is None:
                return []
            for edge in edge_names:
                try:
                    info = edge_fn(sym, edge)
                except Exception:
                    continue
                if info is None:
                    continue
                edge_eV = float(info[0]) if not isinstance(info, (int, float)) else float(info)
                if edge_eV <= 0:
                    continue
                if abs(edge_eV - e_eV) <= window_eV:
                    notes.append(
                        f"  Near {sym} {edge} edge: E_edge≈{edge_eV/1000.0:.4g} keV "
                        f"(working E={energy_keV:.4g} keV, |Δ|≤{window_eV:.0f} eV)"
                    )
        except Exception:
            continue
    return notes
...
class LayerTable(ttk.Frame):
    ...
    def load_common_stack(self):
        self.load_from_spec(
            [
                {"material": "Kapton (C22H10N2O5)", "t_mm": "0.10", "auto": False, "enabled": True},
                {"material": "Air (dry)", "t_mm": "50", "auto": False, "enabled": True},
                {"material": "Sample (current)", "t_mm": "", "auto": True, "enabled": True},
                {"material": "Air (dry)", "t_mm": "50", "auto": False, "enabled": True},
                {"material": "Kapton (C22H10N2O5)", "t_mm": "0.10", "auto": False, "enabled": True},
            ]
        )

    def load_from_spec(self, layers: List[Dict[str, Any]]):
        """Load layers from a list of dicts (session / stack presets)."""
        self.clear()
        for layer in layers or []:
            if not isinstance(layer, dict):
                continue
            self.add_row(
                material=layer.get("material") or self.MATERIALS[0],
                t_mm=str(layer.get("t_mm", "")),
                auto=bool(layer.get("auto", False)),
                enabled=bool(layer.get("enabled", True)),
                rho=layer.get("rho", ""),
                formula=layer.get("formula", ""),
            )

    def export_spec(self) -> List[Dict[str, Any]]:
        """Serialize current layer rows for session save."""
    ...
    def add_row(self, material=None, t_mm="", auto=False, enabled=True, rho="", formula=""):
        idx = len(self.rows)
        ...
        v_rho = tk.StringVar(value=str(rho) if rho not in (None, "") else "")
        v_formula = tk.StringVar(value=str(formula) if formula not in (None, "") else "")
...
class App(tk.Tk):
    ...
    def _build_menu(self):
        ...
        file_menu.add_command(label="Save session…", command=self.save_session_dialog, accelerator="Ctrl+Shift+S")
        file_menu.add_command(label="Load session…", command=self.load_session_dialog, accelerator="Ctrl+O")
        file_menu.add_separator()
        file_menu.add_command(label="Export report...", command=self.export_report, accelerator="Ctrl+S")
        file_menu.add_separator()
        ...
        self.bind("<Control-S>", lambda e: self.save_session_dialog())  # Ctrl+Shift+S on some platforms
        self.bind("<Control-Shift-S>", lambda e: self.save_session_dialog())
        self.bind("<Control-o>", lambda e: self.load_session_dialog())
    ...
    def _on_close(self):
        try:
            ...
            if not maximized:
                ...
                w = self.winfo_width()
                h = self.winfo_height()
                self.config_manager.save_window_state(w, h, False)
            else:
                ws = self.config_manager.load_window_state()
                self.config_manager.save_window_state(ws.get("width", 1120), ws.get("height", 720), True)
        except Exception:
            pass
        self.destroy()
    ...
    def _about(self):
        messagebox.showinfo(
            ...
            "• Element μ/ρ from xraydb (Elam / Chantler)\n"
            "• Alloy & stack presets + session save/load\n"
            "• Layers / detector Q / exposure scaling\n\n"
            "Tip: Use measured density for absolute thickness."
        )
    ...
    def _build_sample_tab(self, tab):
        ...
        preset_card = ttk.LabelFrame(tab, text="Alloy preset", style="Card.TLabelframe")
        preset_card.grid(row=0, column=0, sticky="ew", pady=(0, 10))
        preset_card.columnconfigure(1, weight=1)

        self._alloy_choice_map = {}  # display name -> id
        try:
            choices = alloy_choices()
        except Exception:
            choices = []
        display_names = []
        for aid, name in choices:
            display_names.append(name)
            self._alloy_choice_map[name] = aid
        self.alloy_preset_var = tk.StringVar(value=display_names[0] if display_names else "")
        ttk.Label(preset_card, text="Preset:").grid(row=0, column=0, sticky="w")
        alloy_combo = ttk.Combobox(
            preset_card,
            textvariable=self.alloy_preset_var,
            values=display_names,
            state="readonly",
            width=36,
        )
        alloy_combo.grid(row=0, column=1, sticky="ew", padx=(8, 0))
        load_alloy_btn = ttk.Button(preset_card, text="Load alloy", command=self._load_alloy_preset)
        load_alloy_btn.grid(row=0, column=2, sticky="e", padx=(8, 0))
        ToolTip(alloy_combo, "从内置合金库载入成分（可选密度）。\n载入后仍可手动改。")
        ToolTip(load_alloy_btn, "把选中的合金预设写入成分表，并在有密度时填入 Manual ρ。")

        comp = ttk.LabelFrame(tab, text="Composition", style="Card.TLabelframe")
        comp.grid(row=1, column=0, sticky="ew")
...
留空时，程序会同时输出：
  ...
  - corner limit 的 Qmax（仅作几何极限参考）"""
    ...
    def _build_layers_tab(self, tab):
        ...
        stack_row = ttk.Frame(card)
        stack_row.grid(row=1, column=0, sticky="ew", pady=(0, 8))
        stack_row.columnconfigure(1, weight=1)
        self._stack_choice_map = {}
        try:
            schoices = stack_choices()
        except Exception:
            schoices = []
        snames = []
        for sid, sname in schoices:
            snames.append(sname)
            self._stack_choice_map[sname] = sid
        self.stack_preset_var = tk.StringVar(value=snames[0] if snames else "")
        ttk.Label(stack_row, text="Stack preset:").grid(row=0, column=0, sticky="w")
        stack_combo = ttk.Combobox(
            stack_row,
            textvariable=self.stack_preset_var,
            values=snames,
            state="readonly",
            width=40,
        )
        stack_combo.grid(row=0, column=1, sticky="ew", padx=(8, 0))
        ttk.Button(stack_row, text="Load stack", command=self._load_stack_preset).grid(
            row=0, column=2, sticky="e", padx=(8, 0)
        )
        ToolTip(stack_combo, "内置束路层叠预设（Kapton/Be/石英毛细管等）。")

        self.layers_table = LayerTable(card)
        self.layers_table.grid(row=2, column=0, sticky="ew")
    ...
    def _set_status(self, msg: str, ok: bool = True) -> None:
        self.status.set(msg)
        # Keep a quiet path for non-modal feedback; errors also raise dialogs elsewhere.
    ...
    def _build_help_tab(self, tab):
        ...
        steps = (
            f"MuSlice v{__version__} — 最短路径\n\n"
            "① Beam: 输入线站能量 E(keV)，或波长 λ(Å)。\n"
            "② Sample: 可用 Alloy preset 一键载入（默认 Ti-2448），或手填成分。\n"
            "   • 切换 wt/at 不会自动换算，需要时点 Convert。\n"
            "③ Density: 强烈建议实测 ρ（g/cm³）；含 O/N/C/H/B 时勿迷信估算。\n"
            "④ Target: 选 T 或 μt；新手先看 μt=0.5–1.2 窗口。\n"
            "   • 窗片/空气：勾选 Apply target to total stack，并配置 ⑤ Layers。\n"
            "⑤ Layers: Load stack 载入 Kapton/Be/石英等束路，再改真实厚度。\n"
            "⑥ Compute thickness → 推荐样品厚度 + T_total + T–t 曲线。\n"
            "⑦ Exposure（可选）: 相对曝光缩放。\n"
            "⑧ File → Save session 保存整套设置，线站可复现。\n\n"
            "经验：T≈0.3–0.7；μt≈0.5–1.2。\n"
        )
    ...
    def _load_alloy_preset(self):
        name = self.alloy_preset_var.get()
        aid = self._alloy_choice_map.get(name)
        if not aid:
            messagebox.showinfo(APP_TITLE, "No alloy preset selected.")
            return
        try:
            alloy = alloy_by_id(aid)
            if not alloy:
                raise ValueError(f"Unknown alloy id: {aid}")
            basis = str(alloy.get("basis", "wt"))
            comp = alloy.get("composition") or {}
            percent = {str(k): float(v) for k, v in comp.items()}
            self.fraction_basis.set(basis)
            self.comp_table.clear()
            for sym, val in percent.items():
                self.comp_table.add_row(sym, f"{val:.6g}")
            # normalize display lightly if sum far from 100
            dens = alloy.get("density_g_cm3")
            if dens is not None:
                self.density_mode.set("manual")
                self.density_g_cm3.set(f"{float(dens):.6g}")
            notes = alloy.get("notes") or ""
            self._set_status(f"Loaded alloy preset: {alloy.get('name', aid)}. {notes}".strip(), ok=True)
        except Exception as exc:
            messagebox.showerror(APP_TITLE, f"Failed to load alloy: {exc}")

    def _load_stack_preset(self):
        name = self.stack_preset_var.get()
        sid = self._stack_choice_map.get(name)
        if not sid:
            messagebox.showinfo(APP_TITLE, "No stack preset selected.")
            return
        try:
            stack = stack_by_id(sid)
            if not stack:
                raise ValueError(f"Unknown stack id: {sid}")
            self.layers_table.load_from_spec(stack.get("layers") or [])
            notes = stack.get("notes") or ""
            self._set_status(f"Loaded stack: {stack.get('name', sid)}. {notes}".strip(), ok=True)
        except Exception as exc:
            messagebox.showerror(APP_TITLE, f"Failed to load stack: {exc}")

    def collect_session_dict(self) -> Dict[str, Any]:
        try:
            pairs = self.comp_table.get_pairs(allow_empty=False, normalize_symbols=True)
        except Exception:
            pairs = []
        return {
            "beam": {
                ...
                "wavelength_A": self.wavelength_A.get(),
                "source_db": self.source_db.get(),
                "cross_kind": self.cross_kind.get(),
                "incident_angle_deg": self.incident_angle_deg.get(),
            },
            "sample": {
                "composition": composition_from_pairs(pairs, self.fraction_basis.get()),
                "density_mode": self.density_mode.get(),
                "density_g_cm3": self.density_g_cm3.get(),
                "density_est_model": self.density_est_model.get(),
            },
            "target": {
                "target_mode": self.target_mode.get(),
                "target_T": self.target_T.get(),
                "target_mut": self.target_mut.get(),
                "reco_mut_min": self.reco_mut_min.get(),
                "reco_mut_max": self.reco_mut_max.get(),
                "target_scope_total": bool(self.target_scope_total.get()),
            },
            "detector": {
                "det_enable": bool(self.det_enable.get()),
                "det_L_mm": self.det_L_mm.get(),
                "det_pixel_mm": self.det_pixel_mm.get(),
                "det_nx": self.det_nx.get(),
                "det_ny": self.det_ny.get(),
                "det_x0": self.det_x0.get(),
                "det_y0": self.det_y0.get(),
                "det_rmin_mm": self.det_rmin_mm.get(),
                "det_rmax_mm": self.det_rmax_mm.get(),
            },
            "layers": self.layers_table.export_spec() if hasattr(self, "layers_table") else [],
            "exposure": {
                "exp_ref_time_s": self.exp_ref_time_s.get(),
                "exp_ref_t_sample_mm": self.exp_ref_t_sample_mm.get(),
                "exp_ref_flux": self.exp_ref_flux.get(),
                "exp_ref_Ttotal": self.exp_ref_Ttotal.get(),
                "exp_new_flux": self.exp_new_flux.get(),
                "exp_eff_ratio": self.exp_eff_ratio.get(),
                "exp_snr_ratio": self.exp_snr_ratio.get(),
                "exp_model": self.exp_model.get(),
                "exp_use_computed_t": bool(self.exp_use_computed_t.get()),
                "exp_new_t_sample_mm": self.exp_new_t_sample_mm.get(),
            },
        }

    def apply_session_dict(self, data: Dict[str, Any]) -> None:
        ...
        def _set(var, key, src):
            if key in src and src[key] is not None:
                var.set(src[key])

        _set(self.energy_mode, "energy_mode", beam)
        _set(self.energy_keV, "energy_keV", beam)
        _set(self.wavelength_A, "wavelength_A", beam)
        _set(self.source_db, "source_db", beam)
        _set(self.cross_kind, "cross_kind", beam)
        _set(self.incident_angle_deg, "incident_angle_deg", beam)

        basis, pairs = pairs_from_composition(sample.get("composition") or {})
        if basis:
            self.fraction_basis.set(basis)
        if pairs:
            self.comp_table.clear()
            for sym, pct in pairs:
                self.comp_table.add_row(sym, f"{pct:.6g}")
        _set(self.density_mode, "density_mode", sample)
        _set(self.density_g_cm3, "density_g_cm3", sample)
        _set(self.density_est_model, "density_est_model", sample)

        _set(self.target_mode, "target_mode", target)
        _set(self.target_T, "target_T", target)
        _set(self.target_mut, "target_mut", target)
        _set(self.reco_mut_min, "reco_mut_min", target)
        _set(self.reco_mut_max, "reco_mut_max", target)
        if "target_scope_total" in target:
            self.target_scope_total.set(bool(target["target_scope_total"]))

        if "det_enable" in detector:
            self.det_enable.set(bool(detector["det_enable"]))
        for key, var in [
            ("det_L_mm", self.det_L_mm),
            ("det_pixel_mm", self.det_pixel_mm),
            ("det_nx", self.det_nx),
            ("det_ny", self.det_ny),
            ("det_x0", self.det_x0),
            ("det_y0", self.det_y0),
            ("det_rmin_mm", self.det_rmin_mm),
            ("det_rmax_mm", self.det_rmax_mm),
        ]:
            _set(var, key, detector)

        if "layers" in data and hasattr(self, "layers_table"):
            self.layers_table.load_from_spec(data.get("layers") or [])

        for key, var in [
            ("exp_ref_time_s", self.exp_ref_time_s),
            ("exp_ref_t_sample_mm", self.exp_ref_t_sample_mm),
            ("exp_ref_flux", self.exp_ref_flux),
            ("exp_ref_Ttotal", self.exp_ref_Ttotal),
            ("exp_new_flux", self.exp_new_flux),
            ("exp_eff_ratio", self.exp_eff_ratio),
            ("exp_snr_ratio", self.exp_snr_ratio),
            ("exp_model", self.exp_model),
            ("exp_new_t_sample_mm", self.exp_new_t_sample_mm),
        ]:
            _set(var, key, exposure)
        if "exp_use_computed_t" in exposure:
            self.exp_use_computed_t.set(bool(exposure["exp_use_computed_t"]))

    def save_session_dialog(self):
        path = filedialog.asksaveasfilename(
            title="Save MuSlice session",
            defaultextension=".json",
            filetypes=[("MuSlice session", "*.json"), ("All files", "*.*")],
        )
        if not path:
            return
        try:
            save_session(path, self.collect_session_dict())
            self._set_status(f"Session saved: {path}", ok=True)
            messagebox.showinfo(APP_TITLE, f"Saved session:\n{path}")
        except Exception as exc:
            messagebox.showerror(APP_TITLE, f"Save session failed: {exc}")

    def load_session_dialog(self):
        path = filedialog.askopenfilename(
            title="Load MuSlice session",
            filetypes=[("MuSlice session", "*.json"), ("All files", "*.*")],
        )
        if not path:
            return
        try:
            ...
            self.apply_session_dict(data)
            self._set_status(f"Session loaded: {path}", ok=True)
            messagebox.showinfo(APP_TITLE, f"Loaded session:\n{path}")
        except Exception as exc:
            messagebox.showerror(APP_TITLE, f"Load session failed: {exc}")
    ...
    def compute(self):
        ...
        try:
            ...
            lines = []
            lines.append(f"{__app_name__} v{__version__}  |  {utc_now_iso()}")
            lines.append(f"Energy: {e_keV:.6g} keV   (λ = {lam_A:.6g} Å)")
            ...
            # Density / interstitial warnings
            interstitial_hit = [s for s in w_mass if s in INTERSTITIAL_LIKE]
            if rho_meta is None:
                lines.append(f"Density ρ: {rho:.6g} g/cm³  (manual)")
            else:
                lines.append(f"Density ρ: {rho:.6g} g/cm³  (estimated; model={rho_meta['model']})")
                lines.append("⚠ Density is ESTIMATED — absolute thickness is approximate only.")
            ...
            if interstitial_hit and rho_meta is None:
                lines.append(
                    f"⚠ Composition contains interstitial-like elements ({', '.join(sorted(interstitial_hit))}); "
                    "prefer measured density."
                )

            edge_notes = absorption_edge_notes(list(w_mass.keys()), e_keV)
            if edge_notes:
                lines.append("Absorption-edge proximity:")
                lines.extend(edge_notes)

            lines.append(f"(μ/ρ)_mix: {mu_rho:.6g} cm²/g   [source={source}, kind={kind}]")
            lines.append(f"μ: {mu_mm:.6g} 1/mm   (1/μ = {1.0/mu_mm:.6g} mm = {1000.0/mu_mm:.6g} μm)")
            if abs(tilt_factor - 1.0) > 1e-6:
...
# ---- Detector Q/d coverage (optional, orthogonal geometry) ----
            ...
            if self.target_mode.get() == "T":
                ...
                if self.target_scope_total.get():
                    ...
                    lines.append(
                        f"  sample thickness t = {t_req:.6g} mm ({t_req*1000:.6g} μm)  "
                        f"(sample μt = {mu_eff*t_req:.6g})"
                    )
                    ...
                    lines.append(
                        f"  thickness t = {t_req:.6g} mm ({t_req*1000:.6g} μm)  (μt = {mu_eff*t_req:.6g})"
                    )
            else:
                ...
                if self.target_scope_total.get():
                    ...
                    lines.append(
                        f"  thickness t = {t_req:.6g} mm ({t_req*1000:.6g} μm)  "
                        f"(T = exp(-μt) = {math.exp(-mut):.6g})"
                    )
            ...
            if mut_min and mut_max and mut_min > 0 and mut_max > mut_min:
                ...
                lines.append(
                    f"  thickness ~ {tmin:.6g} – {tmax:.6g} mm  "
                    f"({tmin*1000:.6g} – {tmax*1000:.6g} μm)"
                )
                lines.append(f"  transmission ~ {math.exp(-mut_max):.3g} – {math.exp(-mut_min):.3g}")
                lines.append("")

            lines.append("Common transmission targets (thickness in mm / μm):")
            for T in [0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2]:
                tt = thickness_for_transmission(mu_eff, T)
                lines.append(
                    f"  T={T:.1f} -> t={tt:.6g} mm ({tt*1000:.6g} μm)   (μt={-math.log(T):.3g})"
                )
            lines.append("")
            lines.append("Notes:")
    ...
    def export_report(self):
        ...
        try:
            ...
            if str(path).lower().endswith(".md"):
                body = f"# {__app_name__} report\n\n```\n{self._last_summary.rstrip()}\n```\n"
